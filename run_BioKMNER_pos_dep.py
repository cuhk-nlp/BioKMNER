...
log_file_name = './logs/log-'+now_time+'-dep'
...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, label=None, word=None,
                 syn_feature=None , range_list=None , word_matrix=None, syn_matrix=None):
...
class Pos_attention_Processor(DataProcessor):
    """Processor for the cws POS CTB5 data set."""
    def __init__(self, data_dir, feature_flag, word_threshold, feature_threshold):
    ...
    def _create_examples(self, lines, set_type):
        examples = []
        for i, (sentence, label, word_list, syn_feature_list, range_list) in enumerate(lines):
            guid = "%s-%s" % (set_type, i)
            text_a = ' '.join(sentence)
            text_b = None
            word = '#*#*#'.join(word_list)
            ...
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label, word=word,
                             syn_feature=syn_feature_list,  range_list= range_list))
        return examples
    ...
    def _get_data(self, lines, all_feature_data):
        ...
        if self.feature_flag == 'chunk':
            for (sentence, label), feature_list in zip(lines, all_feature_data):
                ...
                data.append((sentence, label, word_list, syn_feature_list, range_list))
            ...
            for (sentence, label), feature_list in zip(lines, all_feature_data):
                ...
                for token_index, token in enumerate(feature_list):
                    if 'dep' in token:
                        ...
                        word_list.append(current_token)
                        syn_feature_list.append(current_feature)
                        range_list.append(token['governed_index'])
                    else:
                        current_token = '<UNK>'
                        current_feature = '<UNK>'
                        word_list.append(current_token)
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, gram2id, feature2id, max_word_size):
    global countnumber_sentence, countnumber_input, wrong_sentence, max_length_sentence
    ...
    for example in examples:
        text_len.append(len(example.text_a.split(' ')))
    max_seq_length = min(int(max(text_len) * 1.5) + 2, max_seq_length)
    max_word_size = min(max(text_len) + 2, max_word_size)
    # ----
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        range_sw = example.range_list
        max_length_sentence.append(len(wordlist))
        if len(textlist) != len(wordlist):
            wrong_sentence+=1
            print(textlist)
            print(wordlist)
        ...
        for i, word in enumerate(textlist):
            ...
            # labels_mask: [1]
            # valid: [1, 0]
            # -2 because we need to add two tokens [CLS] and [SEP]
        ...
        if len(wordlist)==len(syn_features):
            ...
            if len(word_ids) >= max_seq_length - 1:
                word_ids = word_ids[0:(max_seq_length - 2)]
                feature_ids = feature_ids[0:(max_seq_length - 2)]
        ...
        for i, sw in enumerate(range_sw):
            if i <= (max_seq_length-2) :
                if sw <= (max_seq_length-2):
                    sw_list = []
                    if sw == -1:
                        sw = i
                    sw_list.append(i)
                    sw_list.append(sw)
                    sw_governed = [x for x in range(len(range_sw)) if ( (range_sw[x] == i) & (x < (max_seq_length-2)) )]
                    sw_list.extend(sw_governed)
                    for loc in sw_list:
                        word_matching_matrix[i + 1][loc] = 1
                        syn_matching_matrix[i + 1][loc] = 1
    ...
    # print(len(max_length_sentence))
    # print("max_length: %s" % max(max_length_sentence))
    return features
...
def main():
    ...
    parser.add_argument('--word_threshold', type=int, default=1, help="Can be used for distant debugging.")
    parser.add_argument('--feature_threshold', type=int, default=1, help="Can be used for distant debugging.")
    ...
    processor = Pos_attention_Processor(args.data_dir, args.feature_flag, args.word_threshold, args.feature_threshold)
    ...
    model = NER.from_pretrained(args.bert_model,
                                ...
                                args=args,
                                word_size=len(processor.gram2id)+2,
                                feature_size=len(processor.feature2id)+2)
    if args.restore:
    ...
    if args.do_train:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, start_index in enumerate(tqdm(range(0, len(train_examples), args.train_batch_size))):
                ...
                train_features = convert_examples_to_features(batch_examples, label_list, args.max_seq_length,
                                                              tokenizer,
                                                              processor.gram2id, processor.feature2id,
                                                              args.max_word_size)
                ...
                all_word_matching_matrix = torch.tensor([f.word_matching_matrix for f in train_features],
                                                        dtype=torch.float)
                all_syn_matching_matrix = torch.tensor([f.syn_matching_matrix for f in train_features],
                                                       dtype=torch.float)
                batch = [all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_valid_ids, all_lmask_ids,
                ...
                loss, _ = model(input_ids, segment_ids, input_mask, label_ids, valid_ids, l_mask, word_ids,
                                feature_ids, word_matching_matrix, feature_matching_matrix, num_labels)
            ...
            if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                ...
                for start_index in range(0, len(eval_examples), args.eval_batch_size):
                    eval_batch_examples = eval_examples[start_index: min(start_index + args.eval_batch_size,
                                                                         len(eval_examples))]
                    eval_features = convert_examples_to_features(eval_batch_examples, label_list, args.max_seq_length,
                                                                 tokenizer,
                                                                 processor.gram2id, processor.feature2id,
                                                                 args.max_word_size)
                    ...
                    valid_ids = torch.tensor([f.valid_ids for f in eval_features], dtype=torch.long)
                    l_mask = torch.tensor([f.label_mask for f in eval_features], dtype=torch.long)
                    word_ids = torch.tensor([f.word_ids for f in eval_features], dtype=torch.long)
                    feature_ids = torch.tensor([f.syn_feature_ids for f in eval_features], dtype=torch.long)
                    word_matching_matrix = torch.tensor([f.word_matching_matrix for f in eval_features],
                                                        dtype=torch.float)
                    feature_matching_matrix = torch.tensor([f.syn_matching_matrix for f in eval_features],
                                                           dtype=torch.float)
