...
class BertConfig(object):
    ...
    def __init__(self,
        ...
        if isinstance(vocab_size_or_config_json_file, str) or (sys.version_info[0] == 2
            ...
            print("111", vocab_size_or_config_json_file, type(vocab_size_or_config_json_file))
...
class word_channel_attention(nn.Module):
    def __init__(self, config, args):
        super(word_channel_attention, self).__init__()
        self.config = config
        self.args = args
        self.word_embedding = nn.Embedding(config.word_size, int(config.hidden_size/3), padding_idx=0)
        self.linear_1 = nn.Linear(int(config.hidden_size/3), config.hidden_size, bias=False)

    def forward(self, word_seq, hidden_state, char_word_mask_matrix):
        ...
        # embedding (batch_size, word_seq_len, word_embedding_dim)

        batch_size, character_seq_len, _ = hidden_state.shape
        channel = char_word_mask_matrix.shape[1]

        # print('batch size: ', batch_size)
        # print('character_seq_len: ', character_seq_len)

        embedding = self.word_embedding(word_seq)
        # print('embedding shape:', embedding.shape)

        # tmp (batch_size, word_seq_len, hidden_size)
        tmp = self.linear_1(embedding)

        tmp = tmp.permute(0, 2, 1)

        # tmp_hidden_state = torch.stack([hidden_state] * channel, 1)
        # print('tmp_hidden_state shape:', tmp_hidden_state.shape)
        # print('tmp shape:', tmp.shape)
        # u (batch_size, character_seq_len, word_seq_len)
        u = torch.matmul(hidden_state, tmp)

        u = torch.stack([u] * channel, 1)
        ...
        # attention = nn.Softmax(dim=3)(torch.mul(u, char_word_mask_matrix))

        # print('attention shape', attention.shape)

        # attention (batch_size, channel, character_seq_len, word_seq_len, word_embedding_dim)
        attention = torch.stack([attention] * embedding.shape[-1], 4)

        # print('attention shape', attention.shape)
        embedding = torch.stack([embedding] * channel, 1)
        embedding = torch.stack([embedding] * character_seq_len, 2)
        # print('embedding shape', embedding.shape)

        # character_attention (batch_size, channel, character_seq_len, word_seq_len, word_embedding_dim)
        character_attention = torch.mul(attention, embedding)

        # print('character attention shape', character_attention.shape)

        # character_attention (batch_size, channel, character_seq_len, word_embedding_dim)
        character_attention = torch.mean(character_attention, 3)

        # print('character attention shape', character_attention.shape)

        character_attention = character_attention.permute(0, 2, 1, 3)

        # print('character attention 2 shape', character_attention.shape)

        character_attention = character_attention.flatten(start_dim=2)

        # character_attention (batch_size, character_seq_len, channel * word_embedding_dim)
        # print('character attention 3 shape', character_attention.shape)
...
class word_attention(nn.Module):
    def __init__(self, config, args):
        ...
        self.config = config
        self.args = args
        self.word_embedding = nn.Embedding(config.word_size, int(config.hidden_size), padding_idx=0)
        # self.linear_1 = nn.Linear(int(config.hidden_size/2), config.hidden_size, bias=False)
    ...
    def forward(self, word_seq, hidden_state, word_mask_matrix):
        ...
        # attention (batch_size, character_seq_len, word_seq_len)
        ...
        # attention (batch_size, character_seq_len, word_seq_len, word_embedding_dim)
        attention = torch.stack([attention] * embedding.shape[-1], 3)

        # print('attention shape', attention.shape)
        # embedding = torch.stack([embedding] * channel, 1)
        embedding = torch.stack([embedding] * character_seq_len, 1)
        # print('embedding shape', embedding.shape)
        ...
        # character_attention (batch_size, character_seq_len, word_embedding_dim)
        character_attention = torch.sum(character_attention, 2)
...
#         self.config = config
#         self.args = args
#         self.word_embedding_a = nn.Embedding(config.word_size, config.hidden_size)
#         self.word_embedding_c = nn.Embedding(110, config.hidden_size)
...
#         # embedding_c: (batch_size, character_seq_len, word_seq_len, hidden_size)
...
#         p = nn.Softmax(dim=2)(torch.mul(u, tmp_word_mask_metrix))
#
#         # print('attention shape', attention.shape)
#
#         # p (batch_size, character_seq_len, word_seq_len, hidden_state)
#         p = torch.stack([p] * hidden_state.shape[2], 3)
#
#         # print('attention shape', attention.shape)
#
#         # character_seq_len = p.shape[1]
#
#         # embedding_c = torch.stack([embedding_c] * character_seq_len, 1)
#         # print('embedding shape', embedding.shape)
#
#         # character_attention (batch_size, character_seq_len, word_seq_len, word_embedding_dim)
...
class word_kv_memory(nn.Module):
    def __init__(self, config, args):
        ...
        self.args = args
        self.word_embedding_a = nn.Embedding(config.word_size, config.hidden_size)
        self.word_embedding_c = nn.Embedding(config.word_size, config.hidden_size)
        # self.linear_1 = nn.Linear(config.word_embedding_dim, config.hidden_size, bias=False)
        # self.linear_2 = nn.Linear(config.word_embedding_dim, 64)
...
class BertForTokenClassification(BertPreTrainedModel):
    ...
    ```
    """
    def __init__(self, config, num_labels=2, output_attentions=False, keep_multihead_output=False, args=None):
        ...
        self.word_memory = word_kv_memory(config, args)
        # self.word_attention = word_attention(config, args)
        # self.feature_attention = word_attention(config, args)
